		...
		<dl class="userinfo_box clearfix">
			...
			<dd>
				<table class="form_table" width="100%" cellpadding="0" cellspacing="0">
					<col width="120px" />
					<col />
					<?php if($this->userRow['username']){?>
					<tr>
						<th>用户名：</th>
						<td>
								<?php echo isset($this->userRow['username'])?$this->userRow['username']:"";?>
						</td>
					</tr>
						<?php }?>
					<tr>
						<th>邮箱：</th>
						<td><?php if($this->userRow['email']){?>
								<?php echo isset($this->userRow['email'])?$this->userRow['email']:"";?>
								<a class="blue" href="<?php echo IUrl::creatUrl("/ucenter/toChgEmail1");?>">[修改]</a>
							<?php }else{?>
								<a class="blue" href="<?php echo IUrl::creatUrl("/ucenter/toChgEmail1");?>">[绑定邮箱]</a>
							<?php }?>
					...
					<tr>
						<th>手机：</th>
						<td>
							<?php if($this->userRow['phone']){?>
								<?php echo isset($this->userRow['phone'])?$this->userRow['phone']:"";?>
								<a class="blue" href="<?php echo IUrl::creatUrl("/ucenter/toChgPhone1");?>">[修改]</a>
							<?php }else{?>
								<a class="blue" href="<?php echo IUrl::creatUrl("/ucenter/phone_bind");?>">[绑定手机]</a>
							<?php }?>
						</td>
					</tr>
	...
	<div class="form_content m_10">
		...
		<form action='<?php echo IUrl::creatUrl("/ucenter/info_edit_act");?>' method='post' name='user_info'>
			<table class="form_table" width="100%" cellpadding="0" cellspacing="0">
				...
				<?php if(!$this->userRow['username']){?>
				<tr>
					<th><span class="red">*</span> 用户名：</th><td><input class="normal" type="text" name="username"  alt='请填写用户名' /><label>（注册后可用户登陆，注册后不可修改）</label></td>
				</tr>
				<?php }?>
				<tr>
				...
				<tr><th></th><td><label class="btn"><input type="submit"  class='btn_gray btn_small' value="保存基本信息" /></label></td></tr>
			</table>
		</form>
	</div>
</div>
...
$(function(){
	...
	$('input[name=username]').on('blur',function(){
		...
		//window.realAlert(value);
		if(value.length<2 || value.length>20){
			$(this).next('label').removeClass('valid-msg').addClass('invalid-msg').text('用户名长度须在2-20位之间');
			return false;
		}
		if(checkUsername(value)){
			$(this).next('label').removeClass('valid-msg').addClass('invalid-msg').text('用户名已注册');
			return false;
		}
			$(this).next('label').removeClass('invalid-msg').addClass('valid-msg').text('');
			return false;
		
	})
	$('input[type=submit]').on('click',function(){
		var userSel = $('input[name=username]');
		var username = $.trim($('input[name=username]').val());
		if(username){
			if(checkUsername(username)){
				userSel.next('label').removeClass('valid-msg').addClass('invalid-msg').text('用户名已注册');
				return false;
			}
		}
...
function checkUsername(username){
	var url = '<?php echo IUrl::creatUrl("/ucenter/checkUserIsOne");?>';
	var res = 0;//未占用
	$.ajax({
		type:'post',
		async:false,
