...
<?php $cancle = Order_Class::is_cancle($id);?>
	...
	<div class="content">
		<table class="list_table">
			...
			<tbody>
				...
				<tr>
					...
					<td>￥<?php echo isset($item['delivery_fee'])?$item['delivery_fee']:"";?></td>
					<td>￥<?php echo isset($item['save_price'])?$item['save_price']:"";?></td>
					<td>￥<?php echo isset($item['tax'])?$item['tax']:"";?></td>
		...
		<table style="width:300px;margin-right:20px;" class="border_table f_l">
			...
			<tbody>
				...
				<tr>
					<th>配送保价:</th><td><?php if($if_insured==0){?>不保价<?php }else{?>已保价<?php }?></td>
				</tr>
				<tr>
	...
	<div class="content">
		...
		<table style="width:45%;margin-right:20px;" class="border_table f_l">
			...
			<tbody>
				...
				<tr>
					<th>状态：</th><td><?php echo Order_Class::refundmentText($item['pay_status'],$item['type']);?></td>
				</tr>
				<tr>
...
<div id="tab-6" name="table" style="display:none">
	<div class="content">
		<table class="border_table" style='width:98%'>
			...
			<tbody>
				<?php $query = new IQuery("order_log as ol");$query->where = "ol.order_id = $order_id and ol.order_id = 0";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
...
function initButton()
{
	...
	//完成
	<?php if(in_array($orderStatus,array(11,3,13))){?>
	...
	//发货
	<?php if(in_array($orderStatus,array(1,4,8,9,12))){?>
	$('#to_deliver').removeAttr('disabled');
	$('#to_deliver').removeClass('inactive');
	<?php }?>

	//退款
	<?php if(in_array($orderStatus,array(4,6,9,10,12,13))){?>
