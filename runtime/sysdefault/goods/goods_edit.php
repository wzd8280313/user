...
<body>
	<div class="container">
		...
		<div id="admin_right">
			<script type="text/javascript" charset="UTF-8" src="/iweb2/runtime/_systemjs/editor/kindeditor-min.js"></script><script type="text/javascript" charset="UTF-8" src="/iweb2/runtime/_systemjs/editor/lang/zh_CN.js"></script><script type="text/javascript">window.KindEditor.options.uploadJson = "/iweb2/index.php?controller=pic&action=upload_json";window.KindEditor.options.fileManagerJson = "/iweb2/index.php?controller=pic&action=file_manager_json";</script>
			...
			<div id="table_box_1">
				<table class="form_table">
					...
					<tr>
						<th>商品状态：</th>
						<td>
							<label class='attr'><input type="radio" name="is_del" value="0" checked> 上架</label>
							<label class='attr'><input type="radio" name="is_del" value="2"> 下架</label>
							<label class='attr'><input type="radio" name="is_del" value="3"> 待审</label>
							<label class='attr'><input type="radio" name="is_del" value="4" disabled> 预售</label>
							<label>下架和待审商品不会在前台显示</label>
						</td>
					</tr>
					<tr>
						<th>是否共享：</th>
						...
						<td>
							<div class="con">
								<table class="border_table">
									...
									<tr class='td_c'>
										<td><input type='hidden' class='product_id' name='product_id[<%=i%>]' value="<%=item['id']%>"/>
											<input class="small" name="_goods_no[<%=i%>]" pattern="required" type="text" value="<%=item['goods_no'] ? item['goods_no'] : item['products_no']%>" /></td>
										<%var isProduct = false;%>
										...
										<td><input class="tiny" name="_market_price[<%=i%>]" type="text"  value="<%=item['market_price']%>" /></td>
										<td>
					...
					<tr>
						<th>商品推荐类型：</th>
						<td>
							<?php $query = new IQuery("commend_tags");$query->where = "is_close = 0";$query->order = "sort asc";$items = $query->find(); foreach($items as $key => $item){?>
							<label class="attr"><input name="_goods_commend[]" type="checkbox" value="<?php echo isset($item['id'])?$item['id']:"";?>" /><?php echo isset($item['name'])?$item['name']:"";?></label>
							<?php }?>
						</td>
					</tr>
					...
					<tr>
						...
						<td style="position:relative;">
                            <span style="margin-left:100px;width:18px;overflow:hidden;">
								<select style="width:118px;margin-left:-100px" class="auto" name="brand_id" id="xialakuang" onchange="this.parentNode.nextSibling.value=this.value">
									<option value="0">请选择</option>
									<?php $query = new IQuery("brand");$query->group = "name";$items = $query->find(); foreach($items as $key => $item){?>
									<option value="<?php echo isset($item['id'])?$item['id']:"";?>"><?php echo isset($item['name'])?$item['name']:"";?></option>
									<?php }?>
									<!-- <p><input type="text" name="shangpin" id="chapinpai" onblur="cha();"></p> -->
								</select>
							</span>
							<input type="text"  id="chapinpai" onkeyup="cha();"  autocomplete="off" style="width:100px;height:23px;position:absolute;left:10px;border:1px solid #eee;" placeholder="请输入或选择">
							<ul id="add_list" style="width:100px;height:auto;overflow:auto;"></ul>
						</td>
		
						<script>
							function cha() {
								var inp = $('#chapinpai').val();
								if(inp == ''){
									addList('');
									return false;
								}
								var url = '<?php echo IUrl::creatUrl("/goods/getdata");?>';
								$.ajax({
									type:'post',
									url : url,
									data:'name='+inp,
							...
							function addList(obj){
								// $('#add_list').html('');
								var list_1 = '<li style="width:100px;margin-left:0px;border:1px solid #eee;cursor:pointer;"';
								var list_2 = '</li>';
								var list = '';
								for(var i in obj){
									list += list_1 + ' value="'+ obj[i].id + '" class="edit_list">' + obj[i].name + list_2;
								}
								$('#add_list').html(list);

								$('li.edit_list').click(function(){
									var list_name = $(this).text();
									var id = $(this).attr('value');
									$('#chapinpai').val(list_name);
									$('#xialakuang').val(id);
									$('#add_list').html('');
								});
							}
		...
		<!-- 商品品牌模糊搜索    end         -->
			...
			<div id="table_box_2" cellpadding="0" cellspacing="0" style="display:none">
				<table class="form_table">
					...
					<tr>
						<th>产品简介：</th>
						<td><textarea  name="short_desc" style="width:500px;height:50px;"></textarea></td>
					</tr>
					<tr>
						<th>产品描述：</th>
...
$(function()
{
	...
	//window.realAlert(JSON.stringify(goods));
	var goodsRowHtml = template.render('goodsRowTemplate',{'templateData':[goods]});
	$('#goodsBaseBody').html(goodsRowHtml);
	$('#goodsBaseBody').find('input.product_id').val(0);//商品的货品字段为0

	formObj.init(goods);
	$('#chapinpai').val($('#xialakuang').find("option:selected").text());
...
function checkForm()
{
	...
	if($('input[name^="_goods_category"]').length==0){
		alert('商品分类不能为空');
		return false;
	}
	return true;
