...
<div class="headbar">
	...
	<div class="operating">
		...
		<a href="javascript:void(0);" onclick='all_to_excel()'><button class="operating_btn" type="button" onclick="selectAll('id[]')" "><span class="sel_all">全选</span></button></a>
	...
	<div class="searchbar">
		<form action="<?php echo IUrl::creatUrl("/");?>" method="get" name="searchListForm">
			...
			<select class="auto" name="search[commend_id]">
				...
				<?php $query = new IQuery("commend_tags");$query->where = "is_close = 0";$query->fields = "id,name";$items = $query->find(); foreach($items as $key => $item){?>
	...
	<div class="content">
		<table class="list_table">
			...
			<tbody>
				...
				<tr>
					<td><input name="id[]" type="checkbox" value="<?php echo isset($item['id'])?$item['id']:"";?>" onclick='to_excel(this)' /></td>
					...
					<td>
						<select onchange="changeIsDel(<?php echo isset($item['id'])?$item['id']:"";?>,this)" disabled>
							...
							<option value="presell" <?php echo $item['is_del']==4 ? 'selected':'';?>>预售中</option>
						</select>
					</td>
					...
					<td>
						<?php echo substr($item['create_time'],0,10);?>
					</td>
					<td>
...
<table class="border_table" style="width:100%">
	...
	<%for(var item in templateData){%>
		...
		<tr>
			...
			<td>
				<?php $query = new IQuery("commend_tags");$query->where = "is_close = 0";$items = $query->find(); foreach($items as $key => $tag){?>
				<?php $tid=$tag['id']?>
				<label class="attr"><input type="checkbox" name="data[<%=item['id']%>][]" value="<?php echo isset($tid)?$tid:"";?>" <%if(item['commend'] && item['commend'][<?php echo isset($tid)?$tid:"";?>]){%>checked="checked"<%}%> /><?php echo isset($tag['name'])?$tag['name']:"";?></label>
				<?php }?>
			</td>
		</tr>
	<%}%>
