...
<div class="headbar">
	...
	<ul class='tab' name='conf_menu'>
		...
		<li><a href="javascript:select_tab('products_page_conf');">详情页设置</a></li>
	...
	<div class="content form_content">
		<form action="<?php echo IUrl::creatUrl("/system/save_conf/form_index/base_conf");?>"  method="post" enctype='multipart/form-data' name='base_conf'>
			<table class="form_table">
				...
				<tr>
					<th>顶部广告展示：</th>
					<td><label class='attr'><input type='radio' name='index_ad_button' value="open" />开启</label>
					<label class='attr'><input type='radio' name='index_ad_button' value="close" />关闭</label>
					</td>
				</tr>
				<tr>
					<th></th>
				...
				<tr>
					...
					<td>
						...
						<table class='border_table' id='guide_box'>
							...
							<tr class='td_c'>
								<td><input type='text' name='guide_name[]' class='small' value='<%=name%>'  alt='请填写导航名' /></td>
								<td><input type='text' name='guide_link[]' class='middle' value='<%=link%>'  alt='请填写URL' /></td>
								<td>
		...
		<form action='<?php echo IUrl::creatUrl("/system/save_conf/form_index/products_page_conf");?>' method='post' name='products_page_conf'>
			<table class='form_table'>
				<col width="150px" />
				<col />
				...
				<tr>
					...
					<td>
						...
						<table class='border_table' id='product_page_box'>
							<col width="150px" />
							<col width="250px" />
							<col width="120px" />
							<thead>
							...
							<tr>
								<td colspan='3'>
									<button type='button' class='btn' onclick='add_product()'><span>添加链接</span></button>
								</td>
							</tr>
							</tfoot>

							<!--导航行模板-->
							<script type='text/html' id='productsPageTemplate'>
							<tr class='td_c'>
								<td><input type='text' name='server_name[]' class='small' value='<%=server_name%>'  alt='请填写名称' /></td>
								<td><input type='text' name='server_link[]' class='middle' value='<%=link%>'  alt='请填写URL' /></td>
								<td>
									<img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_asc.gif";?>' alt='向上' title='向上' />
									<img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_desc.gif";?>' alt='向下' title='向下' />
									<img class='operator' src='<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/admin/icon_del.gif";?>' alt='删除' title='删除' />
						...
						<label>设置商品详情页服务内容和链接地址</label>
					</td>
				</tr>
				<tr>
					<th></th>
					<td><button type='submit' class='submit'><span>保存配置</span></button></td>
				</tr>
			</table>
		</form>
				...
				<tr>
					...
					<td>
						...
						<table class='border_table' id='slide_box' >
							...
							<tr class='td_c'>
								<td><input type='text' name='slide_name[]' class='small' value='<%=name%>' pattern='required' /></td>
								<td><input type='text' name='slide_url[]' class='middle' value='<%=url%>' pattern='required' /></td>
								<td><input type='text' name='slide_color[]' class='small' value='<%=color%>' pattern='required' /></td>
		...
		<form action="<?php echo IUrl::creatUrl("?controller=system&action=save_conf&form_index=system_conf");?>" method="post" name='system_conf'>
...
$(function(){
	...
	//详情页内容
	<?php if(isset($this->confRow['product_page']) && $this->confRow['product_page'] ){?>
	<?php $product_info = unserialize(IFilter::stripSlash($this->confRow['product_page']))?>
	var product_info_List = <?php echo JSON::encode($product_info);?>;
	for(var index in product_info_List)
	{
		add_product(product_info_List[index]);
	}
	<?php }else{?>
		add_product();
	<?php }?>
...
function add_product(data){
	var data = data ? data : {};
	var guideTrHtml = template.render('productsPageTemplate',data);
	$('#product_page_box tbody').append(guideTrHtml);
	var last_index = $('#product_page_box tbody tr').size()-1;
	buttonInit(last_index,'#product_page_box');
}
//添加导航
