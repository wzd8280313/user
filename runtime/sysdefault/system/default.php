...
<body>
	<div class="container">
		...
		<div id="admin_right">
			<?php $site_config = new Config('site_config');$config_info = $site_config->getInfo();$store_num_warning = isset($config_info['store_num_warning']) ? $config_info['store_num_warning'] : 20?>
<?php $refund_warning = isset($config_info['refund_warning']) ? $config_info['refund_warning'] : 10;$limit_seconds = 30*24*3600;?>
					...
					<td>
						<table class="list_table2" width="100%">
							...
							<tbody>
								...
								<tr><th>库存预警</th><td><?php $query = new IQuery("goods");$query->fields = "count(id) as countNums";$query->where = "store_nums < $store_num_warning and (is_del = 0 or is_del = 4)";$items = $query->find(); foreach($items as $key => $item){?><a href="javascript:formSubmit('storeNumWarning');"><b class="f14 red3"><?php echo isset($item['countNums'])?$item['countNums']:"";?></b> 个</a><?php }?></td></tr>
								<?php $i=0;?>
								<tr><th>恶意用户</th><td><?php $query = new IQuery("member as m");$query->join = "left join refundment_doc as r on m.user_id = r.user_id";$query->group = "m.user_id";$query->where = "TIMESTAMPDIFF(second,r.time,NOW()) < $limit_seconds";$query->having = "count(r.id) > $refund_warning";$query->fields = "count(m.user_id) as times";$items = $query->find(); foreach($items as $key => $item){?><?php $i++?><?php }?><a href="<?php echo IUrl::creatUrl("/member/bad_member_list");?>"><b class="f14 red3"><?php echo isset($i)?$i:"";?></b> 个</a></td></tr>
