...
<body style="width:650px;min-height:250px;">
	<div class="pop_win">
		...
		<ul class="select">
			...
			<li onmouseover="showCategory(<%=item['id']%>,<%=level%>);"><label><input name="categoryVal" type="<?php echo isset($type)?$type:"";?>" value="<%=item['id']%>" onchange="selectCategory(this);" <%if(checkedCategory.indexOf(item['id']) != -1){%>checked="checked"<%}%> /><%=item['name']%></label></li>
			<%}%>
		</ul>
		</script>
	</div>
