...
?>
<?php if(isset($search['title'])&&$search['title']){?>
<?php $where='title="'.$search['title'].'"'?>
<?php }else{?>
<?php $where=1?>
<?php }?>
<div class="headbar">
	...
	<div class="operating">
		...
		<a href="javascript:void(0)" onclick="delModel({'form':'regiment_del'});"><button class="operating_btn" type="button"><span class="delete">批量删除</span></button></a>
	</div>
<div class="searchbar">
		<form action="<?php echo IUrl::creatUrl("/");?>" method="get" name="">
			<input type='hidden' name='controller' value='market' />
			<input type='hidden' name='action' value='regiment_list' />
			
			名称<input class="small" name="search[title]" type="text" value="" />
...
<div class="content">
	...
	<form method='post' action='<?php echo IUrl::creatUrl("/market/regiment_del");?>' name='regiment_del'>
		<table class="list_table">
			<colgroup>
				<col width="40px" />
				<col />
			...
			<tbody>
				...
				<?php $query = new IQuery("regiment");$query->order = "sort asc,id desc";$query->page = "$page";$query->where = "$where";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					<td><input type='checkbox' name='id[]' value='<?php echo isset($item['id'])?$item['id']:"";?>' /></td>
					<td><a href='<?php echo IUrl::creatUrl("/site/tuan_product/active/".$item['id']."");?>' target='_blank' class="orange" title="<?php echo isset($item['title'])?$item['title']:"";?>"><?php echo isset($item['title'])?$item['title']:"";?></a></td>
