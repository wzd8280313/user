...
<body>
	...
	<?php $conArr = IFilter::act(IReq::get('search'),'strict');$where = getSearchCondition($conArr);?>
<div class="headbar">
	...
	<div class="searchbar">
		<form action="<?php echo IUrl::creatUrl("/");?>" method="get" name="searchListForm">
			...
			<select class="auto" name="search[position_id]">
				...
				<?php foreach(Api::run('getAdPosition') as $key => $item){?>
...
<div class="content">
	<form name='ad' method='post' action='<?php echo IUrl::creatUrl("/tools/ad_del");?>' >
		<table id="list_table" class="list_table">
			...
			<col />
			<tbody>
				<?php $query = new IQuery("ad_manage as ad,ad_position as adp");$query->fields = "adp.name as adp_name,ad.*";$query->where = "$where and ad.position_id = adp.id";$query->order = "ad.order ASC";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
