...
$.idealforms.errors = {
  ...
  required: '不能为空.',
  number: '如无法接收验证码，请确认短信是否被拦截.',
  digits: '必须是唯一的数字.',
  name: '必须至少有3个字符长，并且只能包含字母.',
  username: '必须在32之间和4个字符长，并以字母开头。(.)',
  pass: '6-20位字符，建议由字母，数字和符号两种以上组合.',
  strongpass: '必须至少为8个字符长，至少包含一个大写字母和一个小写字母和一个数字或特殊字符.',
  email: '请填写有效的e -mail地址. <em>(例如：user@gmail.com)</em>',
  phone: '请填写正确的手机号码. <em>(例如：13012345678)</em>',
  zip: 'Must be a valid US zip code. <em>(例如： 33245 or 33245-0003)</em>',
  url: 'Must be a valid URL. <em>(例如： www.google.com)</em>',
