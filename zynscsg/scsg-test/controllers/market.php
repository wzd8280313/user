...
class Market extends IController
{
	...
	public function active_add(){
		...
		$m_active=new IModel('active');
		$dataArray=array(
			//'id'=>$id,
			'goods_id'=>null,
			'type'=>IFilter::act(IReq::get('type','post'),'int'),
			'create_time'=>IFilter::act(IReq::get('start_time','post')),
			'end_time'=>IFilter::act(IReq::get('end_time','post')),
			'is_close'=>IFilter::act(IReq::get('is_close','post'),'int'),
			//'spec'=>IFilter::act(IReq::get('spec','post'),'int'),
			'product_id'=>IFilter::act(IReq::get('product_id','post'),'int'),
			'active_price'=>IFilter::act(IReq::get('active_price','post')),
			
			);
		//and is_close=0 and end_time>"'.$dataArray['start_time'].'" and start_time < "'.$dataArray['start_time']"
		$m_active->getObj('is_close=0 and end_time >"'.$dataArray['create_time'].'" and create_time <"'.$dataArray['create_time'].'"');
			$uploadObj = new PhotoUpload();
			if(isset($_FILES['img']['name']) && $_FILES['img']['name'] != '')
			{
			...
			if(isset($_FILES['banner']['name'])&&$_FILES['banner']['name']!=''){
				...
				//var_dump($photoInfo);
				$dataArray['banner_img']=$photoInfo['banner']['img'];
				$dataArray['bg_img']=$photoInfo['bg']['img'];
			}
			$m_active->setData($dataArray);
			$m_active->add();
